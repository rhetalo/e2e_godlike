...
- `agents.docs/game-panel/` — game panel (ultra.panel) knowledge base + test plan (~58 tests; **Phase 5 закрыта** = IDOR + XSS бэкап/папка/motd/консоль + SQLi; + 09-Jun: FILE редактор/Recycle Bin, Logout-smoke, Audit Log, scheduled-форма структурно. Next — только **деструктив** (version change/rebuild, backups restore, install плагина, scheduled-мутация) — вне рамки «обратимо+recovery», нужен явный risk-decision владельца)
