...
| TC-GP-LOGIN-004 | Невалидный формат email → форма не сабмитится (остаёмся на /login) |
| TC-GP-LOGIN-005 | Сессия переживает перезагрузку (F5) — остаёмся залогинены |
...
Реализовано: **~61 тест**. База (36): Phase 1 (8) + power (3) + console (2) + files (2) + config (2) + players (2) + backups (2) + sharing (5) + Port&Domains (2) + Tasks (2) + role enforcement (3) + security (3). + 07-Jun MCP-сессия (15) + 09-Jun (10: FILE-005/006, SEC-004, AUTH-004, SEC-005, SHR-006, BKP-003, LOGIN-004/005/006). (+ storefront LOC-001/002/003 — отдельно, не в этом счёте.)
...
**09-Jun (+10 тестов, ~61 всего; + storefront LOC×3 отдельно):** LOGIN-004 (невалидный email-формат), LOGIN-005 (сессия переживает F5), LOGIN-006 (неавторизованный `/server/{uuid}` → `/login`, access-control) — FILE-005 (CodeMirror-редактор: open `server.properties` через «...»→Open → `.cm-content` непуст → выход без сохранения, read-only) + FILE-006 (Recycle Bin: delete→trash→**Restore**→root→delete, self-cleaning) + **SEC-004** (XSS/SQLi в config `motd`: автосейв→round-trip как текст→откат; закрывает config-инпут Phase 5) + **AUTH-004** (`@smoke`: Log Out из аккаунт-меню → редирект `/login`, изолированный контекст; закрывает дыру в smoke-срезе) + **SEC-005** (`@regression`, online: XSS в выводе консоли через `say` → эхо как текст, без диалога; **закрывает Phase 5**) + **SHR-006** (`@regression`, online: Audit Log фиксирует `server:power.start` после Start; recovery=ensureOffline) + **BKP-003** (`@regression`, **структурно**: scheduled-форма Schedule Backup + Set interval + секция Scheduled; мутацию create→delete НЕ делаем — рекуррентный cron на shared-проде, контрол удаления не разведан). Зелёные, `tsc`=0. ⚠️ Recon-гоча (KB §9c): в корзине **Restore — ОТДЕЛЬНАЯ кнопка** (`button:has-text("Restore")`, вне `action-btn-group` с Download/Move/Duplicate/Delete; disabled до выбора строки). Дубли тест-папок копятся в корзине между прогонами — авто-очистка 24ч.
...
| TC-GP-BKP-003 ✅(структ.) | **Scheduled backup**: форма Schedule Backup + Set interval + секция Scheduled | структурный | @regression | ✅ done 09-Jun (без сабмита). Мутация create→delete отложена: рекуррентный cron, удаление не разведано |
