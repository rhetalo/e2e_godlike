...
> ⚠️ **18-Aug-2026 — дрейф тест-сервера.** Сервер `93521c70`, на котором снят этот recon,
> **удалён**. Тесты `mods.spec` / `plugins.spec` переехали на `GAME_PANEL_PLUGIN_SERVER_UUID`
> (дефолт `0c743c25`, выделен владельцем).
> **Как узнать этот симптом:** падение выглядит как таймаут — клик по type-фильтру «Plugins»
> не находит элемент (в call log НЕТ `locator resolved to`), а `waitForResponse` на
> `/minecraft/{plugins,mods}` не видит запроса вообще. Это значит `/extensions` не отрендерил
> каталог. **Сначала проверь, жив ли сервер**, и только потом подозревай тайминги.
> Селекторы/контракты ниже при этом остаются валидными — они не зависят от конкретного сервера.
