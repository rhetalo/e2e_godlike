...
| **vps/panel** | ✅ done | network (8→3, MERGE), storage (REWRITE→реальные данные диска), login (networkidle×6→waitForURL, REWRITE редиректа, MERGE disabled-кнопок), server (raw→PO, getByText, ~20→12), options (12→4, raw под-табы→PO), media (console+skip), power.actions (19→6, SUITE 1–4→test.step, confirm→PO-геттеры, console(50)+waitForTimeout убраны, setTimeout 180s). **rebuild** дроблён: `vps.panel.rebuild.spec` 700 стр → helpers + os-selection(2) + selection(3) + **real(SUITE7-деструктив, отдельный файл, интент не тронут)**; waitForTimeout×8/networkidle/console убраны. Все не-деструктивные зелёные на живой панели; **real.spec НЕ запускался** (реальный реинсталл ОС — отдельное решение владельца). login/power.actions/server/rebuild убраны из ESLint legacy-fence. |
| **game/panel** | ✅ done | tag-convention уже узаконен (describe-уровень, §3.1 — без правок кода). Raw-локаторы вынесены в PO: login «My Servers» → `GamePanelDashboardPage.heading`; server.overview name/address/uuid → `GamePanelServerPage.nameLabel/addressLabel/uuidLabel`; sharing SHR-004 → `srv.nameLabel`. login(6)+server.overview(3) зелёные; sharing-правка идентична по семантике (мутирующий role-change не гонялся). Деструктив game-панели (version change/restore/install) — вне рамок, нужен risk-decision владельца. |

**✅ Программа унификации завершена — все 6 доменов done (16-Jun-2026).** Остаток вне рамок:
деструктивные тесты, требующие явного risk-decision владельца — `funnels/registration-flow`
(создаёт юзера на проде, §3.2), `vps.panel.rebuild.real` (реальный реинсталл ОС, отрефакторен но
не запускался), деструктив game-панели (version change/restore/install). Их интент НЕ трогали.
