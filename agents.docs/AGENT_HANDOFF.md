...
> ### ⚠️⚠️ `.env` КОДОМ НЕ ЧИТАЕТСЯ (проверено 18-Aug-2026)
>
> В проекте **нет `dotenv`**, нет `--env-file` в npm-скриптах и нет `globalSetup`. Значения
> из `.env` попадают в тесты **только** если их экспортировал шелл (`set -a; . .env; set +a`
> в `runner.sh` на VPS, или CI/CD Variables). Во всех остальных случаях работают **фолбэки в
> коде** (`process.env.X ?? "фолбэк"`).
>
> Сейчас `.env` и фолбэки совпадают один-в-один — поэтому разницы никто не замечал, и это
> ловушка: **правка `.env` может не повлиять на прогон вообще.** Меняешь сервер / креды →
> меняй **фолбэк в коде** (`utils/gameAuth.ts` · `utils/auth.ts` · `fixtures/test-data.ts`),
> а `.env`/`.env.example` обновляй следом, чтобы не разъезжались. Решение владельца
> (18-Aug-2026): `dotenv` НЕ подключаем — фолбэки в коде остаются источником правды.
