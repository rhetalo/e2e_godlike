...
## 10. Правки июнь 2026 — сессия 9 (waitForTimeout финальная фаза: 25 вхождений)

### Цель
Убрать все оставшиеся `waitForTimeout` из 5 целевых файлов. До сессии: ~35 вхождений. После: ~10 (только `vps.build.spec.ts` и `VpsPanelServerDetailPage.ts`).

### Замены по файлу

#### `pages/VpsConfigPage.ts` — 4 вхождения ✅

| Метод | Было | Стало |
|---|---|---|
| `selectLocation` (400ms) | `waitForTimeout(400)` | `.configure-server__location-active[hasText=name].waitFor('visible')` |
| `selectOsType` (300ms) | `waitForTimeout(300)` | `activeOsType.locator('.configure-server__type_title', {hasText}).waitFor('visible')` |
| `openOsDropdown` (300ms) | `waitForTimeout(300)` | `osDropdownItems.first().waitFor('visible')` |
| `selectOsVersion` (300ms) | `waitForTimeout(300)` | `.custom-dropdown__selected-content span[hasText=version].waitFor('visible')` |

Note: `expect` не добавлялся в импорт — все замены используют `.waitFor()` напрямую на локаторе.

#### `pages/MobileCartPage.ts` — 4 вхождения ✅

| Метод | Было | Стало |
|---|---|---|
| `selectGameByChip` (1000ms) | `waitForTimeout(1000)` | Удалён (`waitForFunction` выше уже ждёт disabled→enabled) |
| `selectGameBySearch` (500ms) | `waitForTimeout(500)` | Удалён (аналогично) |
| `selectPlan` (500ms) | `waitForTimeout(500)` | `expect(ramDropdown.customSelectSelected).toContainText(planName, {timeout:5_000})` |
| `selectBillingPeriod` (500ms) | `waitForTimeout(500)` | `expect(billingDropdown.customSelectSelected).toContainText(label, {timeout:5_000})` |

#### `tests/vps/panel/vps.panel.power.actions.spec.ts` — 5 вхождений ✅

| Место | Было | Стало |
|---|---|---|
| `confirmModal()` (300ms) | после `data-bs-dismiss` click | `expect(sp.activeModal).not.toBeVisible({timeout:5_000})` |
| T6.2 cancel (400ms) | перед `expect(activeModal).not.toBeVisible` | Удалён — следующая строка уже ждёт |
| T6.3 cancel (400ms) | то же | Удалён |
| T6.4 cancel (400ms) | то же | Удалён |
| T6.5 cancel (600ms) | то же | Удалён |

#### `tests/vps/panel/vps.panel.rebuild.spec.ts` — 6 вхождений ✅

| Место | Было | Стало |
|---|---|---|
| `openRebuildPage` (1000ms) | после `networkidle` | `rebuildPage.allOsCards.first().waitFor('attached', timeout:5_000)` |
| accordion expand test (500ms) | после `debianBtn.click()` | `expect(debianBtn).not.toHaveClass(/collapsed/, {timeout:5_000})` |
| OS card click (500ms) ×2 | после `almaCard.click()` | `expect(almaCard).toHaveClass(/selected-card/, {timeout:5_000})` |
| OS card click (400ms) | первый клик в single-select test | то же |
| OS card click (500ms) | `debianCard.click()` в single-select test | `expect(debianCard).toHaveClass(/selected-card/, {timeout:5_000})` |

#### `tests/vps/funnel/vps.funnel.spec.ts` — 6 вхождений ✅

| Место | Было | Стало |
|---|---|---|
| Billing caption loop (400ms) | после `selectCycle(label)` в for-loop | Удалён — `toContainText` auto-retries |
| total1m price read (400ms) | после `selectCycle("1 Month")` | `expect(activePeriod()).toContainText("1 Month", {timeout:3_000})` |
| total12m price read (400ms) | после `selectCycle("12 Months")` | `expect(activePeriod()).toContainText("12 Months", {timeout:3_000})` |
| dropdown lastItem click (300ms) | перед `orderServerType.toContainText` | Удалён — `toContainText` auto-retries |
| Happy Path billing (300ms) | после `selectCycle("1 Month")` | `expect(activePeriod()).toContainText("1 Month", {timeout:3_000})` |
