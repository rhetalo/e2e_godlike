...
for (const gameName of gamesToTest) {
  test(`Validate promocode for game (new): ${gameName}`, async ({ browser }) => {
    test.setTimeout(60000);
    console.log(`\n===== START NEW TEST FOR GAME: ${gameName} =====`);
    ...
    try {
      ...
      for (let i = 0; i < tariffCount; i++) {
        ...
        await test.step(
          `Tariff "${tariffTitle}" for game "${gameName}"`,
          async () => {
            console.log(
      ...
      console.log(`\n===== END NEW TEST FOR GAME: ${gameName} =====`);
    } catch (error) {
      console.log(`[ERROR] New test failed for game ${gameName}: ${error}`);
      throw error;
    } finally {
      await page.close();
      await context.close();
