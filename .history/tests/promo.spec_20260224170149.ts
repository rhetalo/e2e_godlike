...
const gamesToTest = [
  'Hytale', 'Minecraft', 'Minecraft Bedrock', 'Vintage Story', 
...
test.describe('Promocode validation - stable TS', () => {
  ...
  // -------------------------
  test('Login to godlike.host', async ({ browser }) => {
  ...
  // -------------------------
  // Проверка промокодов для каждой игры
  // -------------------------
  for (const gameName of gamesToTest) {
    test(`Validate promocode for game: ${gameName}`, async () => {
      ...
      // -------------------------
      const gameLink = page.locator(`a.game__title:text-is("${gameName}")`).first();
      await expect(gameLink).toBeVisible({ timeout: 30000 });
      await gameLink.click();

      // -------------------------
      // Находим тарифы с кнопкой Add to Cart
      // -------------------------
      ...
      for (let i = 0; i < tariffCount; i++) {
        ...
        const tariffTitleLocator = currentTariff.locator('xpath=../../..').locator('.storefront__tariff-title');
        const tariffTitle = (await tariffTitleLocator.textContent())?.trim() || `Tariff #${i + 1}`;
        ...
        // -------------------------
        await page.goBack();
        tariffs = page.locator(tariffSelector);
        await tariffs.first().waitFor({ state: 'visible', timeout: 30000 });
      }
