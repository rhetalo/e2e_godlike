...
const gamesToTest = [
  ...
  'Minecraft', // Java
  'Minecraft Bedrock',
  'Rust','Terraria','Valheim','7 Days to Die','DayZ'
];
...
test.describe('Promocode validation - simple stable version', () => {
  ...
  for (const gameName of gamesToTest) {
    ...
    test(`Validate promocode for game: ${gameName}`, async ({ page }) => {
      ...
      // строгое совпадение имени, чтобы избежать дублей
      let gameLink: Page | undefined;

      if (gameName === 'Minecraft') {
        // точное различение Java/Bedrock по href
        ...
        // для остальных игр - точное совпадение текста
