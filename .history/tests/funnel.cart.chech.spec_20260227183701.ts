...
test("test Credit/Debit Card (Stripe", async ({ page }) => {
  ...
  // // Ждем появления контейнера оплаты
  // const panelBody = page.locator(".panel-body");
  // await expect(panelBody).toBeVisible({ timeout: 15000 });

  // // Ждем рендер Stripe контейнера
  // const stripeElements = panelBody.locator("#stripeElements");
  // await expect(stripeElements).toBeVisible({ timeout: 15000 });

  // // Проверяем контейнер поля карты
  // const cardContainer = panelBody.locator("#stripeCreditCard");
  // await expect(cardContainer).toBeVisible({ timeout: 15000 });

  // // Проверяем iframe номера карты
  // const cardNumberFrame = page.frameLocator(
  //   'iframe[title*="ввода номера карты"]',
  // );
  // await expect(cardNumberFrame.locator("input")).toBeVisible({
  //   timeout: 15000,
  // });

  // // Проверяем iframe срока действия
  // const expiryFrame = page.frameLocator('iframe[title*="срока действия"]');
  // await expect(expiryFrame.locator("input")).toBeVisible({
  //   timeout: 15000,
  // });

  // // Проверяем iframe CVC
  // const cvcFrame = page.frameLocator('iframe[title*="CVC"]');
  // await expect(cvcFrame.locator("input")).toBeVisible({
  //   timeout: 15000,
  // });

  // Проверяем кнопку оплаты внутри panel-body
  const payButton = page.getByRole('button[type="submit"]');
  await page.getByRole("button", { class: "LinkButton" });
