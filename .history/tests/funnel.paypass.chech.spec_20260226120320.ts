...
test("test checkout", async ({ page }) => {
  ...
  // ШАГ 11: Выбор второго способа оплаты (radio button)
  const paymentRadios = page.locator("ins.iCheck-helper");

  // убеждаемся, что радиобатны есть
  await expect(paymentRadios).toHaveCountGreaterThan(1);

  // кликаем ВТОРОЙ радиобатн
