...
test("test paypass", async ({ page }) => {
  ...
  // ШАГ 13: Проверка, что iframe PayPal появился
  const paypalFrame = page
    .locator(
      'iframe[name="__zoid__paypal_buttons__eyJzZW5kZXIiOnsiZG9tYWluIjoiaHR0cHM6Ly9nb2RsaWtlLmhvc3Q"',
    )
    .contentFrame();

  // ШАГ 14: Проверка кнопки PayPal внутри iframe
  await expect(
    paypalFrame!.getByRole("link", { name: "Оплатить с помощью PayPal" }),
