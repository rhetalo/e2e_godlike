...
test("test Credit/Debit Card (Stripe", async ({ page }) => {
  ...
  await expect(cardExpiry).toHaveValue(/12\s?\/\s?28/);

  // CVC
  await cardCvc.fill("123");
  await expect(cardCvc).toHaveValue("123");

  // ===== Проверка кнопки LinkPay =====

  const linkPayButton = page.locator('.LinkButton-inner:has-text("Pay with")');
  await expect(linkPayButton).toBeVisible();
});
