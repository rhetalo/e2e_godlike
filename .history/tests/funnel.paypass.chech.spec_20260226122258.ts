...
test("test paypass", async ({ page }) => {
  ...
  // ШАГ 11: Выбор способа оплаты (PayPal)
  const paymentRadios = page.locator(
    'input[type="radio"][name="paymentmethod"]',
  );

  // ждем, пока JS дорисует способы оплаты
  await paymentRadios.first().waitFor({
    state: "attached",
    timeout: 20000,
  });

  // убеждаемся, что способы оплаты есть
  const radiosCount = await paymentRadios.count();
  expect(radiosCount).toBeGreaterThan(0);

  // выбираем PayPal
  await paymentRadios
    .filter({ has: page.locator('[value="paypal_ppcpv"]') })
    .first()
    .check({ force: true });

  // ШАГ 12: Проверка, что появилась кнопка PayPal
