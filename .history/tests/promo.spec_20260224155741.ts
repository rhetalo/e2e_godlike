...
test.describe('Promocode validation - stable TS', () => {
  ...
  for (const gameName of gamesToTest) {
    ...
    test(`Validate promocode for game: ${gameName}`, async ({ page }) => {
      ...
      await Promise.all([
        page.waitForURL('**/clientarea/clientarea.php', { timeout: 150000 }),
        page.click('#login')
      ]);
      ...
      await expect(gameLink).toBeVisible({ timeout: 100000 });
      ...
      await page.waitForSelector(tariffSelector, { timeout: 150000 });
      ...
      for (let i = 0; i < tariffCount; i++) {
        ...
        // Ждём, пока промокод появится или явно проверяем отсутствие
        const promoLabel = page.locator('.promocode__label-success');
        let promoText = 'Invalid or missing';

        if (await promoLabel.isVisible({ timeout: 50000 }).catch(() => false)) {
