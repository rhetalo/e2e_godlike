...
test.describe('Promocode validation - stable TS', () => {
  ...
  for (const gameName of gamesToTest) {
    ...
    test(`Validate promocode for game: ${gameName}`, async ({ page }) => {
      ...
      await page.locator(tariffSelector).first().waitFor({ state: 'visible', timeout: 20000 });
      ...
      for (let i = 0; i < tariffCount; i++) {
        const currentTariff: Locator = page.locator(tariffSelector).nth(i);

        console.log(`\n[TARIFF] Game: ${gameName} | Tariff #${i + 1}`);

        // wait for element to be visible, scroll + click
        await currentTariff.waitFor({ state: 'visible', timeout: 15000 });
