...
test.beforeAll(async ({ browser }) => {
  ...
  await page.goto(`${BASE_URL}/clientarea/login`, { waitUntil: 'domcontentloaded', timeout: 6000 });
...
for (const gameName of gamesToTest) {
  test(`Validate promocode for game: ${gameName}`, async ({ browser }) => {
    ...
    try {
      ...
      await page.goto(`${BASE_URL}/game-servers-en/`, { waitUntil: 'networkidle', timeout: 6000 });
      ...
      const gamePageUrl = await gameLink.getAttribute('href') || `${BASE_URL}/game-servers-en/`;
      ...
      await tariffLocator.first().waitFor({ state: 'visible', timeout: 6000 });
