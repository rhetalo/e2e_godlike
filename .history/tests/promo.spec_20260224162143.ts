...
test.describe('Promocode validation', () => {
  ...
  for (const gameName of gamesToTest) {
    ...
    test(`Validate promocode for game: ${gameName}`, async ({ page }) => {
      ...
      for (let i = 0; i < tariffCount; i++) {
        ...
        const isVisible = await promoLabel.isVisible({ timeout: 5000 }).catch(() => false);
        const promoText = isVisible ? (await promoLabel.textContent())?.trim() : 'Invalid promocode';
        ...
        // ASSERT VALIDITY
        // -------------------------
        expect(promoText).toMatch(/^Activated promocode:/);
