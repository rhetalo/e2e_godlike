...
test("test Credit/Debit Card (Stripe", async ({ page }) => {
  ...
  // ШАГ 13: Ожидание появления полей Stripe (Hosted Fields)
  ...
  // ШАГ 14: Валидация ввода

  // 1. Номер карты принимает только цифры
  await cardNumber.fill("abcd1111");
  await expect(cardNumber).toHaveValue("1111");

  // 2. Валидный тестовый номер Stripe
  ...
  // 4. Проверка CVC
  await cardCvc.fill("12");
  await expect(cardCvc).toHaveValue("12");

  await cardCvc.fill("123");
  await expect(cardCvc).toHaveValue("123");

  // ШАГ 15: Проверка кнопки оплаты через LinkPay
